...
实施前基线（2026-09-13，T00）的历史测试数字见 [`docs/baseline-audit.md`](docs/baseline-audit.md)（历史快照；其中 3 个失败为 T04 已修复的静默降级子用例）。当前官方数字以 [`docs/test-evidence.json`](docs/test-evidence.json) 的最近一次带日期记录为准（由 pytest 实际输出解析生成，不手写；完整套件，未排除 tests/test_cli.py）；CI 矩阵以 push 结果为准。
