...
def backfill_repair_queues(runs_dir: Path, *, dry_run: bool = False, force: bool = False, limit: int = 0) -> dict[str, Any]:
    ...
    for run_dir in run_dirs:
        ...
        target_json = run_dir / REPAIR_QUEUE_JSON
        target_md = run_dir / REPAIR_QUEUE_MD
        ...
        if dry_run:
            ...
            continue
        try:
            queue = write_repair_queue_artifacts(_topic_from_state(run_dir), run_dir)
        ...
        report["items"].append(
            _item(
                ...
                target_json,
                status=queue.status,
                total=_safe_int(queue.summary.get("total")),
                block=_safe_int(queue.summary.get("block")),
                high=_safe_int(queue.summary.get("high")),
                medium=_safe_int(queue.summary.get("medium")),
                blocks_submission=_safe_int(queue.summary.get("blocks_submission")),
                blocks_downstream=_safe_int(queue.summary.get("blocks_downstream")),
            )
        )
    return report


def render_repair_queue_backfill_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# Repair Queue Backfill",
        "",
    ...
    if items:
        lines.extend(["## Runs", "", "| Run | Action | Status | Total | Block | High | Medium | Blocks Submission | Blocks Downstream |", "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |"])
            ...
            lines.append(
                ...
                + " | ".join(
                    [
                        ...
                        str(item.get("total") or 0),
                        str(item.get("block") or 0),
                        str(item.get("high") or 0),
                        str(item.get("medium") or 0),
                        str(item.get("blocks_submission") or 0),
                        str(item.get("blocks_downstream") or 0),
                    ]
                )
                + " |"
            )
