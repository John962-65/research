...
def _package_specs() -> list[dict[str, Any]]:
    return [
        ...
        {"source": "10-agent-deliberation.md", "dest": "audits/agent-deliberation.md", "required": False, "note": "确定性角色规则投影与人工复核记录；不代表独立 Agent verdict，独立 verdict 见 independent-deliberation。"},
        {"source": "10-agent-deliberation.json", "dest": "audits/agent-deliberation.json", "required": False, "note": "结构化角色规则汇总，independent_agent_execution 恒为 false。"},
        {"source": "10-independent-deliberation.md", "dest": "audits/independent-deliberation.md", "required": False, "note": "各角色独立 LLM verdict；仅 multi_agent 启用时生成。"},
        {"source": "10-independent-deliberation.json", "dest": "audits/independent-deliberation.json", "required": False, "note": "结构化独立 verdict，每条绑定 ledger call id、model、revision 与 prompt/response 哈希。"},
        {"source": "10-gate-decision.md", "dest": "audits/gate-decision.md", "required": True, "note": "最终 Gate 裁决：本次投稿包是否允许 publishable，含阻断/警告来源与人工覆盖记录。"},
        {"source": "10-gate-decision.json", "dest": "audits/gate-decision.json", "required": True, "note": "结构化最终 Gate 裁决，聚合确定性审计、独立 verdict、缺失角色与人工覆盖。"},
