...
class ResultValidationTest(unittest.TestCase):
    ...
    def test_posthoc_contract_change_blocks_with_binding_mismatch(self) -> None:
        # A12：结果出来后改契约 → 执行绑定摘要不一致 → 阻断，旧批准不沿用。
        plan = _plan()
        results = [
            _result("candidate", 0, {"success_rate": 0.8, "runtime_cost": 0.3}),
            _result("baseline", 0, {"success_rate": 0.7, "runtime_cost": 0.4}),
        ...
        self.assertTrue(any("契约摘要" in item for item in report["blocking_issues"]))
        # 绑定一致时不阻断。
